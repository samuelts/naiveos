...
struct multiboot_t {
    ...
    uint32_t drivers_length;    // 指出第一个驱动器这个结构的大小
    uint32_t drivers_addr;      // 指出第一个驱动器结构的物理地址
 ...
 * base_addr_low 是启动地址的低 32 位，base_addr_high 是高 32 位，启动地址一共有 64 位。
 * length_low 是内存区域大小的低 32 位，length_high 是内存区域大小的高 32 位，总共 64 位。
 * type 是相应地址区间的类型，1 代表可用 RAM, 所有其他值代表保留区域
 */
typedef
struct mmap_entry_t {
    uint32_t size;              // size 是除 size 外自身变量的大小
